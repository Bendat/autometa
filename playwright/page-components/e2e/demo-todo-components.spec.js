...
test.describe("New Todo", () => {
    test("should allow me to add todo items", async ({ todoPage: { newTodo, todoTitles: todoTitle, localStorageCount }, }) => {
    ...
    test("should clear text input field when an item is added", async ({ todoPage: { newTodo, localStorageCount }, }) => {
        await newTodo.create(TODO_ITEMS[0]);
        await newTodo.expect.toBeEmpty();
        await localStorageCount(1);
    });
    test("should append new items to the bottom of the list", async ({ todoPage: { localStorageCount, newTodo, todoCount: { expect: expectTodo }, todoTitles: { expect: expectTitle }, }, }) => {
...
test.describe("Mark all as completed", () => {
    ...
    test("should allow me to mark all items as completed", async ({ todoPage: { markComplete, items: { expect: expectItems }, localStorageCount, }, }) => {
        await markComplete.check();
        await expectItems.toHaveClass(["completed", "completed", "completed"]);
        await localStorageCount(3);
    });
    test("should allow me to clear the complete state of all items", async ({ todoPage: { markComplete, items }, }) => {
        await markComplete.check();
        await markComplete.uncheck();
        await items.expect.toHaveClass(["", "", ""]);
    });
    test("complete all checkbox should update state when items are completed / cleared", async ({ todoPage: { markComplete, localStorageCount, items }, }) => {
