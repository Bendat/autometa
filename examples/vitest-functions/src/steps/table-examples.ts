...
Then("the table can be materialised as instances", (world) => {
  ...
  const items = table.asInstances(InventoryRow, {
  ...
  ensure(items, { label: "Materialised inventory rows." })
    .toBeInstanceOf(Array)
    .toHaveLength(table.rowCount())
    .tap((materialised, context) => {
      ensure(context.isNot).toBe(false);
      ensure(
        materialised.every((row) => row instanceof InventoryRow),
        { label: "Materialised rows should be InventoryRow instances." },
      ).toBeTruthy();
    });
    ...
    .pluck("quantity", { label: "Inventory quantity should be a number." })
    .each((quantity) => quantity.toBeTypeOf("number"));
});

Then("the table preserves raw values when raw true", (world) => {
  const table = world.runtime.requireTable("horizontal");
  const [first] = table.records<TableRecord & { readonly count: string }>({
    raw: true,
  });

  if (!first) {
    throw new Error(
      "Expected at least one record when materialising with raw=true.",
    );
  }
...
Then("each recipe should exist in the recipe catalog", (world) => {
  ...
  if (recipes.length !== 2) {
    throw new Error(
      `Should have 2 recipes registered, but got ${recipes.length}.`,
    );
  }
});

Then(
  "the recipe {string} should list {string} as an addition",
  (recipeName, addition, world) => {
    const recipes = world.scenario.recipes;
    if (!recipes) {
      throw new Error("Recipe catalog should be initialized.");
    }
...
Then(
  "the recipe {string} should store the tasting notes",
  (recipeName, world) => {
...
When("I calculate brew ratio for {string}", (beanName, world) => {
  ...
  const data = table.getRecord(0) as {
    "coffee grams": number;
    "water grams": number;
  };
